...
const myblockchainContract = new web3.eth.Contract(
  myblockchainAbi,
  contractAddress
);
...
router.post("/", async function (req, res, next) {
  try {
    const gas = await myblockchainContract.methods
      .addProcess(
        req.body.harvestId,
        req.body.productId,
        req.body.name,
...
//Get process
router.get(
  "/detail/:harvestId/:productId/:processId",
...
router.get("/list/:harvestId/:productId", async function (req, res, next) {
  try {
    const productInfo = await myblockchainContract.methods
      .getProduct(req.params.harvestId, req.params.productId)
      .call();
    ...
    for (let i = 0; i < amount; i++) {
      const result = await myblockchainContract.methods
        .getProcess(req.params.harvestId, req.params.productId, i)
        .call();
      const return_result = {
        id: i,
        name: result["0"],
